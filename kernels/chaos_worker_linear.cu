...
extern "C" __global__ void crack_kernel(
    ...
    // 2. LINEAR MODE (Feistel disabled for Smart Mapper efficiency)
    // In linear mode, we search passwords in order: a, b, c, ..., aa, ab, ac, ...
    // This allows Smart Mapper to find common passwords first!
    uint64_t password_index = my_linear_id;
